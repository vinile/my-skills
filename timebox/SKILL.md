...
- CHECK 阶段收集每个时间盒结果后，将结果追加写入当日日志
- REVIEW 阶段将 CPR 复盘报告写入当日日志
- **不记录时间盒的启动事件**，只记录结果和复盘
- **任何日志写入失败，仅提示，不阻断主流程**
...
4. **时间盒内不干预，不记录**：RUN 阶段 AI 保持沉默，不写日志，等用户主动回来
5. **CHECK 保持轻量**：单盒收集最多 1 轮对话，不展开分析；收集结果后写入每日总结
6. **复盘才是深度分析的时机**：规律总结、原因分析统一放到全天 REVIEW
7. **日志只记两次**：PLAN 确认后写今日规划，REVIEW 完成后写每日总结 + CPR 复盘
8. **不阻断流程**：日历/日志写入失败等外部错误，提示后继续执行
